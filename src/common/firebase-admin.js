...
const getActiveVariantSchedule = async () => {
  const db = await getAdminDatabase();
  await db.ref("data/activeVariantId").once(
    "value",
    async snap => {
      activeVariantId = snap.val();
      if (activeVariantId === 2) {
        activeVariantId = 0;
      } else {
        activeVariantId++;
      }
      await setActiveVariantSchedule();
    },
    err => {
...
const setActiveVariantSchedule = async () => {
  let scheduleChanged = false;
  const db = await getAdminDatabase();
  await db.ref(`data`).update({ activeVariantId }, err => {
    if (!err) {
